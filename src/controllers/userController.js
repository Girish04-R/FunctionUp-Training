...
const LoginUser=async function(req,res){
  ...
  const User=await userModel.findOne({emailId: userEmail, password: userPass})
  if(!User){
    res.send({status: false,msg : "username and password is not correct"})
  }
  ...
  },"functionup-technetium-girish-very-very-secret-key");
...
const updateuserData=async function(req,res){
  const userId=req.params.userid
  const data=req.body
let updatedUser = await userModel.findByIdAndUpdate(userId,{$set : data}, {new:true});
...
const deleteUserData=async function(req,res){
  ...
  const userDelete = await userModel.findByIdAndUpdate(userId,{$set:{isDeleted:true}},{new:true})
        res.send({status:true, msg: userDelete})
} 
